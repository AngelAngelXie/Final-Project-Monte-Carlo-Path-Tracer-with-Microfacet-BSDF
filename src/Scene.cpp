...
float Scene::castRay(const Ray &ray, int depth,
    ...
    //Vector2f st;
    //Vector3f baseColor = Vector3f(1.0f); // Default to white (no change)

    //inter.obj->getSurfaceProperties(inter.coords, ray.direction, inter.index, inter.uv, inter.normal, st);

    // Use pattern if this triangle says so
    //auto tri = dynamic_cast<Triangle*>(inter.obj);
    //if (tri && tri->usePattern) {
     //   baseColor = tri->evalDiffuseColor(st);
    //}
